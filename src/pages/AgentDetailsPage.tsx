...
export default function AgentDetailsPage() {
  ...
  if (isLoading) {
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="mt-4 text-sm opacity-50 font-medium animate-pulse">{t('loading', 'Loading...')}</p>
      </div>
    );
  }
  ...
  if (error || !agent) {
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[400px] text-red-500">
        ...
        <Link to="/agents" className="mt-4 text-[#d32f2f] flex items-center gap-2 hover:underline">
  ...
  const getStatusBadge = (status: string) => {
    const styles: Record<string, string> = {
      ...
      correction_required: "bg-blue-50 text-blue-700 border-blue-100 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-900/30",
    };
    ...
    return (
      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border shadow-sm ${statusObj}`}>
      ...
      <div>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="flex items-start gap-6">
            ...
            <div className="pt-2">
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm font-medium opacity-60">
                <span className="flex items-center gap-1.5"><Mail size={14} /> {agent.email}</span>
                <span className="w-1 h-1 rounded-full bg-current opacity-30" />
                <span className="flex items-center gap-1.5">
                  <Globe size={14} /> {agent.preferred_language?.toUpperCase()} {t('field_agent')}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <span className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border transition-all ${
              agent.is_active 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-(--card) rounded-3xl border border-(--border) p-6 shadow-sm">
            <h3 className="text-xs font-black uppercase tracking-widest opacity-40 mb-6">{t('agent_overview')}</h3>
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-black text-(--foreground)">{agent.submissions_count}</p>
                    <p className="text-[10px] font-bold opacity-50 uppercase">{t('total_submissions')}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold opacity-50 uppercase">{t('onboarded_on')}</p>
                  </div>
                </div>
              </div>

              <div className="pt-6 border-t border-(--border)">
                <div className="flex items-center justify-between group cursor-help" title="Combined activity metrics">
                  <p className="text-xs font-bold opacity-60">{t('status_history')}</p>
        ...
        <div className="lg:col-span-2">
          <div className="bg-(--card) rounded-3xl border border-(--border) overflow-hidden shadow-sm">
            <div className="p-6 border-b border-(--border) flex items-center justify-between">
              <h3 className="font-black text-(--foreground) tracking-tight">{t('recent_submissions')}</h3>
                ...
                <tbody className="divide-y divide-(--border)">
                  {agent.submissions.length === 0 ? (
                    ...
                    agent.submissions.map((submission) => (
                      <tr key={submission.id} className="hover:bg-(--input)/30 transition-colors group">
                        <td className="px-6 py-4 font-bold text-xs tracking-wider">{submission.bulgarian_id}</td>
                        <td className="px-6 py-4 font-bold text-(--foreground) tracking-tight">
                          {submission.full_name}
                        </td>
                        <td className="px-6 py-4">
                          <span className="text-xs font-medium opacity-70">
                            {isBG ? submission.course.name_bg : submission.course.name_en}
                          </span>
                        </td>
