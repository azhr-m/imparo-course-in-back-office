...
const CopyButton = ({ text, fieldId, copiedId, onCopy, tooltip }: CopyButtonProps) => (
  <button
    ...
    title={tooltip}
  >
    {copiedId === fieldId ? <Check size={14} className="text-emerald-500" /> : <Copy size={14} />}
  </button>
);
...
export default function SubmissionDetailsPage() {
  ...
  if (isLoading) {
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="mt-4 text-sm opacity-50 font-medium animate-pulse">{t('loading')}</p>
      </div>
    );
  }
  ...
  if (error || !submission) {
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[400px] text-red-500">
        ...
        <Link to="/submissions" className="mt-4 text-[#d32f2f] flex items-center gap-2 hover:underline font-bold">
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "processed": return <CheckCircle2 size={16} />;
      case "rejected": return <XCircle size={16} />;
      case "correction_required": return <AlertTriangle size={16} />;
      ...
      <div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="flex items-center gap-6">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wider border shadow-sm ${STATUS_STYLES[submission.status]}`}>
          ...
          <div className="flex items-center gap-4">
            {(submission.status === "pending" || submission.status === "correction_required") && (
              <button
                onClick={() => setDecisionModal({ 
                  show: true, 
                  selectedStatus: submission.status === 'correction_required' ? 'correction_required' : null, 
              ...
              >
                ...
                {submission.status === 'correction_required' ? t("update_decision") : t("make_decision")}
              </button>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-(--card) rounded-3xl border border-(--border) p-8 shadow-sm">
            <h3 className="text-xs font-black uppercase tracking-widest opacity-40 mb-8">{t('applicant_info')}</h3>
            
            <div className="space-y-6">
              <div className="space-y-2">
                <p className="text-[10px] font-black opacity-40 uppercase tracking-widest">{t('full_name')}</p>
                <div className="flex items-center justify-between p-3.5 bg-(--input) rounded-xl font-bold text-sm">
                  ...
                  <CopyButton text={submission.full_name} fieldId="name" copiedId={copiedId} onCopy={handleCopy} tooltip={t('copy')} />
                </div>
              </div>

              <div className="space-y-2">
                <p className="text-[10px] font-black opacity-40 uppercase tracking-widest">{t('bulgarian_id')}</p>
                <div className="flex items-center justify-between p-3.5 bg-(--input) rounded-xl font-mono font-bold text-sm">
                  ...
                  <CopyButton text={submission.bulgarian_id} fieldId="id" copiedId={copiedId} onCopy={handleCopy} tooltip={t('copy')} />
                </div>
              </div>

              <div className="space-y-2">
                <p className="text-[10px] font-black opacity-40 uppercase tracking-widest">{t('phone')}</p>
                <div className="flex items-center justify-between p-3.5 bg-(--input) rounded-xl font-bold text-sm">
                  <span className="flex items-center gap-2"><Phone size={14} className="opacity-50" /> {submission.phone || 'N/A'}</span>
                  {submission.phone && (
                    <CopyButton text={submission.phone} fieldId="phone" copiedId={copiedId} onCopy={handleCopy} tooltip={t('copy')} />
                  )}
                </div>
              </div>

              <div className="space-y-2">
                <p className="text-[10px] font-black opacity-40 uppercase tracking-widest">{t('date_of_birth')}</p>
                <div className="flex items-center justify-between p-3.5 bg-(--input) rounded-xl font-bold text-sm">
                  <span className="flex items-center gap-2"><Calendar size={14} className="opacity-50" /> {submission.date_of_birth || 'N/A'}</span>
                  {submission.date_of_birth && (
                    <CopyButton text={submission.date_of_birth} fieldId="dob" copiedId={copiedId} onCopy={handleCopy} tooltip={t('copy')} />
                  )}
                </div>
              </div>

              <div className="space-y-2">
                <p className="text-[10px] font-black opacity-40 uppercase tracking-widest">{t('place_of_birth')}</p>
                <div className="flex items-center justify-between p-3.5 bg-(--input) rounded-xl font-bold text-sm">
                  <span className="flex items-center gap-2"><MapPin size={14} className="opacity-50" /> {submission.place_of_birth}</span>
                  <CopyButton text={submission.place_of_birth} fieldId="pob" copiedId={copiedId} onCopy={handleCopy} tooltip={t('copy')} />
                </div>
              </div>

              <div className="space-y-2">
                <p className="text-[10px] font-black opacity-40 uppercase tracking-widest">{t('country_of_birth')}</p>
                <div className="flex items-center justify-between p-3.5 bg-(--input) rounded-xl font-bold text-sm">
                  <span className="flex items-center gap-2"><Globe size={14} className="opacity-50" /> {submission.country_of_birth || 'N/A'}</span>
                  {submission.country_of_birth && (
                    <CopyButton text={submission.country_of_birth} fieldId="cob" copiedId={copiedId} onCopy={handleCopy} tooltip={t('copy')} />
                  )}
                </div>
              </div>

              <div className="space-y-2">
                <p className="text-[10px] font-black opacity-40 uppercase tracking-widest">{t('residential_address')}</p>
                <div className="flex items-center justify-between p-3.5 bg-(--input) rounded-xl font-bold text-sm">
                  <span className="truncate text-xs">{submission.residential_address}</span>
                  <CopyButton text={submission.residential_address} fieldId="address" copiedId={copiedId} onCopy={handleCopy} tooltip={t('copy')} />
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-(--border)">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-black opacity-40 uppercase tracking-widest">{t('submitted_by')}</p>
                    <p className="text-sm font-black text-(--foreground)">{submission.agent.name}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-(--card) rounded-3xl border border-(--border) p-8 shadow-sm">
            <h3 className="text-xs font-black uppercase tracking-widest opacity-40 mb-6">{t('admin_notes')}</h3>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="bg-(--card) rounded-3xl border border-(--border) p-8 shadow-sm h-full">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-xs font-black uppercase tracking-widest opacity-40">{t('review_documents')}</h3>
                ...
                <div key={doc.id} className="space-y-3 group text-left">
                  <div className="flex items-center justify-between px-1">
                    <p className="text-xs font-black uppercase tracking-tight opacity-70">
                      {t(`document_type_${doc.document_type}`, doc.document_type.replace(/_/g, ' '))}
                    </p>
                    <a 
                      href={doc.view_url} 
                      target="_blank" 
          ...
          <div className="bg-(--card) w-full max-w-lg rounded-2xl shadow-2xl border border-(--border) overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8">
              <div className="flex justify-between items-center mb-8">
                <h3 className="text-2xl font-black tracking-tight">{t("make_decision")}</h3>
                <button
                  onClick={() => setDecisionModal(prev => ({ ...prev, show: false }))}
              ...
              {!decisionModal.selectedStatus ? (
                <div className="grid grid-cols-1 gap-4">
                  <p className="text-sm opacity-70 mb-2">{t("select_decision_action")}</p>
                  
                  {[
                    { id: 'processed', icon: CheckCircle2, color: 'emerald', label: t('status_processed'), desc: t('mark_as_successfully_verified') },
                    { id: 'correction_required', icon: AlertTriangle, color: 'purple', label: t('status_correction_required'), desc: t('request_agent_to_fix_errors') },
                    { id: 'rejected', icon: XCircle, color: 'red', label: t('status_rejected'), desc: t('reject_due_to_major_issues') }
                  ].map((act) => (
                    <button
                      key={act.id}
                      onClick={() => setDecisionModal(prev => ({ ...prev, selectedStatus: act.id }))}
                      ...
                      `}
                    >
                      <div className={`p-2.5 rounded-lg text-white shadow-lg transition-transform
                      ...
                      <div className="text-left text-(--foreground)">
                        <div className="font-black text-lg leading-none mb-1">{act.label}</div>
                        <div className="text-xs opacity-60 font-medium">{act.desc}</div>
                      </div>
                    </button>
                  ))}
                </div>
                ...
                <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
                  <div className="flex items-center gap-3 p-3 rounded-xl bg-(--input)/50 border border-(--border)">
                    ...
                    <button onClick={() => setDecisionModal(p => ({ ...p, selectedStatus: null }))} className="text-xs text-blue-600 font-bold hover:underline">
                      {t("change")}
                    </button>
                  </div>

                  <div className="space-y-2">
                    <label className="block text-xs font-black uppercase tracking-widest opacity-40 ml-1">{t("admin_notes")}</label>
                    <textarea
                      autoFocus
                      value={decisionModal.notes}
                      onChange={e => setDecisionModal(p => ({ ...p, notes: e.target.value }))}
                  ...
                  <div className="flex gap-4 pt-6">
                    <button
                      onClick={() => setDecisionModal(p => ({ ...p, selectedStatus: null }))}
                      className="flex-1 px-6 py-4 rounded-xl font-bold border border-(--border) hover:bg-(--input) transition-all active:scale-95"
                    >
                      {t("back")}
                    </button>
                    <button
                      onClick={() => updateStatus.mutate({ status: decisionModal.selectedStatus!, notes: decisionModal.notes })}
                      ...
                      className={`flex-2 px-8 py-4 rounded-xl font-black text-white shadow-xl transition-all flex items-center justify-center gap-2 active:scale-95
                        ${decisionModal.selectedStatus === 'processed' ? 'bg-emerald-600 shadow-emerald-500/20' : 
                          decisionModal.selectedStatus === 'rejected' ? 'bg-red-600 shadow-red-500/20' : 'bg-purple-600 shadow-purple-500/20'}
                      `}
                    >
                      {updateStatus.isPending ? <Loader2 className="animate-spin" size={20} /> : t("confirm_decision")}
                    </button>
                  </div>
                </div>
              )}
