...
export default function DashboardPage() {
  ...
  const stats = {
    today: safeSubmissions.filter((s: Submission) =>
      s.created_at?.startsWith(today)
    ).length,
    pending: safeSubmissions.filter((s: Submission) => s.status === "pending")
      .length,
    processed: safeSubmissions.filter(
      (s: Submission) => s.status === "processed"
    ).length,
    activeAgents:
      safeAgents.filter((a: Agent) => a.is_active !== false).length || 0,
  };
...
function StatCard({ title, value, icon, color, loading }: StatCardProps) {
  ...
  return (
    <div className="bg-(--card) p-6 rounded-2xl border border-(--border) shadow-sm hover:shadow-md transition-shadow relative overflow-hidden group">
      <div
        className={`absolute top-0 right-0 w-24 h-24 rounded-full blur-2xl -z-10 translate-x-10 -translate-y-10 transition-transform group-hover:scale-150 ${colorMap[color].split(" ")[0]}`}
      />
